  ...
  try {
    ...
    if (address) {
      if (typeof address != "object") {
        return res.status(400).send({
          status: false,
          message: "Address  is required in the object form",
        });
      }
    }

    if (address && !pincodeRegex.test(address.pincode)) {
      return res
        .status(400)
    ...
    if (!mobileRegex.test(phone)) {
      return res.status(400).send({
        status: false,
        message:
          "please provide  a valid 10 digits phone number starts from 0-9.",
      });
    }
    if (!passwordRegex.test(password)) {
      return res.status(400).send({
        status: false,
        message:
          "Use the strong password with min length 8 and max length 15. And password should include atleast one capital letter and one small letter",
      });
    }
    ...
    if (duplicateEmail) {
      return res.status(400).send({
        status: false,
        message:
          "Email address alerady exists. please use another email address",
      });
    }
    ...
    if (duplicatephone) {
      return res.status(400).send({
        status: false,
        message: "phone number alerady exists. please use another phone number",
      });
    }
