...
const createBook = async function (req, res) {
  try {
    ...
    let { title, excerpt, userId, ISBN, category, subcategory, isDeleted } =
      bookData;
    ...
    if (!isValid(excerpt))
      return res
        .status(400)
        .send({ status: false, message: "Excerpt required." });
...
const getBooksByQuery = async function (req, res) {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, msg: "Book list", data: books });
  } catch (err) {
    return res
      .status(500)
      .send({ status: false, message: "Error", error: err.message });
...
const getBooksById = async function (req, res) {
  try {
    ...
    if (!isValidObjectId(bookId))
      return res.status(400).send({
        status: false,
        message: "invalid Book Id... Plz Enter the valid BookId",
      });
    ...
    if (!bookData)
      return res.status(404).send({
        status: false,
        message: "No such document found or it may be delete",
      });
    ...
    return res
      .status(200)
      .send({ status: true, message: "Books List", data: bookandReview });
  } catch (err) {
    return res
      .status(500)
      .send({ status: false, message: "Error", error: err.message });
  ...
  try {
    ...
    if (!isValidRequestBody(reqbody)) {
      return res.status(400).send({
        status: false,
        message:
          "Enter the details(title,excerpt,release date,ISBN)that you would like to update",
      });
    }
    ...
    if (ISBN && !ISBNRegex.test(ISBN)) {
      return res
        .status(400)
        .send({ status: false, message: "Enter the valid ISBN" });
    }
    if (ISBN && isDuplicateISBN) {
      return res
        .status(400)
        .send({ status: false, message: `ISBN ${ISBN} already exists` });
    }

    if (excerpt && !isValid(excerpt)) {
      return res
        .status(400)
        .send({ status: false, message: "Enter the valid excerpt" });
    }
    ...
    if (releasedAt && !dateRegex.test(releasedAt)) {
      return res.status(400).send({
        status: false,
        message: "Enter the date in the valid format YY-MM-DD",
      });
    }

    const newData = await BookModel.findOneAndUpdate(
...
const deleteBook = async function (req, res) {
  try {
    ...
    if (!isValidObjectId(bookId))
      return res.status(400).send({
        status: false,
        message: "Invalid Book Id. please Enter the valid Book Id",
      });
    ...
    if (!getbook)
      return res.status(404).send({
        status: false,
        message: "No document found or may be deleted",
      });
