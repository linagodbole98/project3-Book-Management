...
const updateRview = async function (req, res) {
  try {
    ...
    if (!isValidObjectId(reviewId))
      return res
        .status(400)
        .send({ status: false, message: "Enter the valid review Id" });

    if (!isValidRequestBody(data))
      return res.status(400).send({
        status: false,
        message: "Enter the details you want to update",
      });
    ...
    if (!isbookId)
      return res.status(404).send({
        status: false,
        Message: "No Book data found or may be deleted",
      });
    ...
    if (!checkreview)
      return res.status(404).send({
        status: false,
        Message: "No review data found or may be deleted",
      });

    // Reviewed By
    if (reviewedBy && !valid(reviewedBy)) {
      return res
        .status(400)
        .send({ status: false, message: "Enter the valid Reviewers name" });
    }
    ...
    if (review && !reviewRegex.test(review)) {
      return res.status(400).send({
        status: false,
        message: "not valid syntax enter the valid review",
      });
    }
    ...
    if (rating && !ratingRegex.test(rating)) {
      return res.status(400).send({
        status: false,
        message: "Enter the valid rating which is between 1 to 5",
      });
    }
    ...
    if (bookId != getId)
      return res.status(404).send({
        status: false,
        Message:
          "bookId doesn't matches with the bookId in the review document ",
      });
    ...
    res.status(200).send({
      status: true,
      message: "updated review data successfully",
      data: isbookId,
      UpdateReview,
    });
  } catch (err) {
...
const deleteRview = async function (req, res) {
  ...
  try {
    ...
    const decount = await BookModel.findOneAndUpdate(
